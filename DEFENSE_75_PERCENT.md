...
Fatigue is dangerous because you cannot see it. The bridge looks fine on the outside, but inside, tiny cracks are slowly growing. One day, the damage reaches a breaking point, and the bridge fails. This has happened in real life — bridges have collapsed because of fatigue that nobody could see.

Testing fatigue on a real bridge is very expensive and takes many years. We cannot break a real bridge just to study it. That is why we built a **computer simulation** — a program that copies how fatigue damage works, so we can study it safely, quickly, and for free.

**The core problem:** How does repeated loading cause cumulative fatigue damage in a bridge beam, and at what point does the structure fail?
...
| **Fatigue Damage State** | State Entity | Damage level per segment, from 0.0 (no damage) to 1.0 (failed) |
...
- **Is damage >= failure threshold?** → If YES → go to DISPOSE
- **Is damage < failure threshold?** → If YES → continue to PROCESS
- **Additional status checks:** SAFE (< 25%), LOW DAMAGE (25-50%), WARNING (50-75%), CRITICAL (75-99%), FAILED (≥ 100%)
...
| `failure_threshold` | 1.0 | The damage level at which the bridge fails |
...
**If damage ≥ failure_threshold → that segment has FAILED → simulation stops.**
...
| 500 | 0.500 (50%) | **1.000 (100%)** | 0.500 (50%) | **FAILED** |

**Result:** Center segment fails at exactly **cycle 500**. Left and Right segments are at 50% damage.
...
- **failure_threshold = 1.0** → Represents 100% structural capacity; at 1.0, the segment breaks
- **segment_factors = [0.5, 1.0, 0.5]** → Center segment is twice as vulnerable as the edges
...
### Finding 1: Center Segment Fails First
The center segment (factor = 1.0) accumulates damage **twice as fast** as the left and right segments (factor = 0.5). This matches real-world bridge behavior — the center of a beam experiences the highest bending stress.
...
Segments with higher factors fail earlier. If we change factors to `[1.0, 1.0, 1.0]`, all segments fail at the same time. If we change to `[0.2, 1.0, 0.8]`, the center still fails first, but the right segment is close behind.

## 4.4 Preliminary Conclusions

1. **Fatigue is predictable** — with known parameters, we can calculate exactly when failure will occur
2. **Location matters** — segments under more stress (higher factor) fail sooner
3. **Small damages add up** — even 0.002 per cycle eventually reaches failure after 500 cycles
...
| **Status Indicator** | Middle-center-left | Overall beam status (SAFE/WARNING/CRITICAL/FAILED) with color |
...
- **Higher value (e.g., 0.004)** → More damage per cycle → Bridge fails faster → Fewer cycles to failure
...
### 6.2 `failure_threshold` — Breaking Point
...
**What is it?**  
This is the **maximum damage a segment can take** before it breaks. Think of it as the bridge's total health points. When damage reaches this number, that segment has failed.

**How does it affect the simulation?**
- **Higher value (e.g., 2.0)** → Bridge can take more damage before breaking → Lasts longer → More cycles
- **Lower value (e.g., 0.5)** → Bridge breaks with less damage → Fails faster → Fewer cycles
- The value 1.0 represents **100% of structural capacity** — a normalized scale

**Real-world meaning:** A higher threshold represents a stronger bridge (better materials, better design). A lower threshold represents a weaker or older bridge.
...
**How does it affect the simulation?**
- Center factor is highest (1.0) → Center always fails first with default settings
- If you set all factors to 1.0 → All segments take equal damage → All fail at the same time
- If you set Left to 2.0 → Left segment gets **double** the base damage → Left fails first
...
| 5 | **No repair or recovery** | Once damage is accumulated, it never decreases. There is no way to simulate bridge repair or maintenance during the simulation. |
...
5. Failure occurs at a **single threshold** value — there is no partial failure or gradual collapse
...
**Speaking Script:**
> "So what is the problem? Every day, cars and trucks drive over bridges. Each vehicle puts a small force on the bridge. One car alone does not break the bridge. But after thousands and thousands of cars, over many years, the small forces add up. This is called fatigue. The scary thing about fatigue is that you cannot see it. The bridge looks normal on the outside, but inside, tiny cracks are growing. We cannot test this on a real bridge because it is too expensive and takes too long. So I built a computer simulation — a program that copies how fatigue works so we can study it safely, quickly, and for free."
...
| `failure_threshold` | 1.0 | Breaking point |
...
- `status[i]` — SAFE / WARNING / CRITICAL / FAILED

**Output Variables (results):**
- Final damage per segment, cycles to failure, saved PNG charts

**Speaking Script:**
> "The simulation has three types of variables. First, input variables — these are the settings that the user can change. The damage increment is 0.002, which means each cycle adds a tiny bit of damage. The failure threshold is 1.0 — when damage reaches this number, that part of the bridge is broken. The segment factors are 0.5, 1.0, and 0.5, which means the center part gets the most damage. The animation speed is 20 milliseconds per frame — this is just how fast the animation looks. And cycles per frame is 2, which means we skip every other cycle to make the animation shorter. Second, state variables — these change while the simulation is running. The cycle counter goes up by 1 every loop. The damage for each segment goes up a little every cycle. The status changes from SAFE to WARNING to CRITICAL to FAILED as damage increases. Third, output variables — these are the results. The final damage of each segment, the cycle when the bridge broke, and the charts that are saved as picture files."
...
**Speaking Script:**
> "This is what the simulation looks like when it runs. At the top, you see the bridge. It has concrete piers holding it up, a road on top with dashed lines, railings on the sides, and water underneath. There is even a small red truck on the road to show the load. Below the road deck are the 3 beam segments — Left, Center, and Right. These segments change color as they get damaged. Green means safe, yellow means a little damage, orange means warning, and red means it is about to break or already broken. Below the bridge, there are 4 small panels: a damage gauge that fills up like a health bar, a status box that says SAFE or WARNING or FAILED, an info panel showing the current cycle and parameters, and a segment detail panel showing the exact percentage for each segment. At the very bottom, there is a line chart that shows how damage grows over time."
...
- Failure condition: `If damage ≥ 1.0 → FAILED`
- Center segment: factor = 1.0 (most vulnerable — gets full damage)
- Edge segments: factor = 0.5 (less vulnerable — gets half damage)
- Runs continuously until failure is detected
- With defaults: Center fails at exactly cycle 500

**Speaking Script:**
> "Let me explain how the math works. It is actually very simple. Every cycle, I use this formula: damage equals damage plus damage increment times segment factor. So if the damage increment is 0.002 and the segment factor is 1.0, then that segment gets 0.002 damage added every cycle. For the center segment, with a factor of 1.0, after 500 cycles the damage is 0.002 times 500, which equals 1.0. And 1.0 is our failure threshold, so the center breaks at cycle 500. For the left and right segments, the factor is only 0.5, so they only get 0.001 damage per cycle. After 500 cycles, they are at 0.5, which is only 50 percent. So the center always fails first because it has the highest factor. This makes sense in real life too — the middle of a bridge bends the most under heavy trucks."
...
**On the Slide:**
- **Finding 1:** Center segment always fails first (factor 1.0 vs 0.5)
...
**Speaking Script:**
> "From the results, I found 4 important things. Finding 1: The center segment always fails first because it has the highest damage factor. This matches real-world bridge behavior. Finding 2: The damage grows in a straight line — it increases by the same amount every cycle. This is called linear damage, and it follows a rule in engineering called Miner's Rule. Finding 3: If I double the damage increment from 0.002 to 0.004, the bridge breaks at 250 cycles instead of 500. That is exactly half. So more damage per cycle means a shorter life. Finding 4: If I double the failure threshold from 1.0 to 2.0, the bridge lasts 1000 cycles instead of 500. So a stronger bridge lasts longer. My conclusion is that the simulation correctly follows Miner's Rule — the damage adds up in a straight line, and we can predict exactly when failure will happen."
...
- Center segment fails first (highest factor)
...
**Speaking Script:**
> "To wrap up: fatigue is damage that adds up from repeated loads. My simulation uses a simple formula — damage equals damage plus increment times factor — to model this. At 75 percent, I have a fully working simulation with a settings window, animated bridge visualization, and saved charts. The center segment fails first because it has the highest factor, and the results match Miner's Rule, which says damage adds up in a straight line. For the remaining 25 percent, I will add more analysis, improve the documentation, and make refinements. That is the end of my presentation. Thank you for listening, and I am ready for your questions."
